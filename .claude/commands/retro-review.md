...
- 「任務 5（停損政策）依 docs/30 §3 現行版本回答：`delay_cost_pct`（延遲成本）與
  `fwd_r5_pct`/`fwd_r20_pct`（停損後有沒有反彈回本）是兩個獨立母體，分開報 n 與平均/中位，
  不要混算。母體都是 0 才寫『未取得』，**不要**自己去讀價格快取臨場算一個數字頂替——那個
  數字沒有 `make` 目標可重現，以後會被誤引成正式產物。」

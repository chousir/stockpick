...
### R5：與現有 2.8 雷達整合（並列 → 漸進）（預估 1.5h）
- **目標**：用 R2 校準參數升級 `grouping.py` 的 `lead_score`，**先並列對照**，確認再決定是否替換。
- **可動檔案**：`src/tw_screener/analysis/grouping.py`（`lead_score` 旁加校準版欄位）、
  `report/group_report.py`（Section 2.8 並列顯示新舊）、`tests/analysis/`。
- **成功標準**：
  - [ ] Section 2.8 同時顯示舊 `lead_score` 與新「資金流向 lead」排名，供人工對照。
  - [ ] 不刪舊邏輯、不破壞 `make week`。
  - [ ] 文件記錄兩者差異與何時用哪個。
- **驗收指令**：`make week GROUP=defg` → 比對 Section 2.8 新舊排名差異。
- **產物**：補強後的雷達；是否完全替換留給使用者拍板。

...
- **Part C 離線可重放 `screen doctor --replay`**：讀 settings 指定的 committed fixture 跑 `diagnose_html`，驗 parser 沒退化、不打網（CI 友善）。`--save-fixture` 在 live OK 時把探針 HTML 落地供手動刷新黃金樣本（**不在每次抓取自動寫 fixtures**，避免 repo churn）。
...
> 動機：所有訊號都在個股/族群層，缺市場層的多空/位階剎車——空頭或高位期照推 breakout 危險。補一個市場層姿態訊號，**定位＝輔助姿態揭露，不硬性 gate 掉訊號（守 playbook/60-analyst-persona.md「由人決策」）**。
...
> 動機：把 [docs/14](14-entry-ladder-portfolio-fix.md) D4「因子簇上限」目前**只在 prompt 層的人工檢核**（[docs/11:202](11-propicks-analysis.md#L202)）落成**可計算模組**——揭露 picks/holdings 五檔其實押同一題材/事件的隱性集中。**定位＝風險揭露，非硬約束（守 playbook/60-analyst-persona.md「由人決策」）。**
...
## D/E/F/G 全面脫離 Goodinfo 重新設計（docs/31）— 2026-08-22

Goodinfo 篩選器本週（W34）被 Cloudflare 全面擋下。使用者駁回「等資料源解封」框架，指示用
現有本地資料重新設計 D/E/F/G，找成長股(潛力股)與左側交易股。研究已完成（**docs/31**，含
W21-33 歷史實證：195 個大型股週冠軍樣本、401 個落難股票週樣本），核心發現：篩選層對大型股
漲多無歧視性排除（問題是召回率普遍低）；F 策略估值門檻是最大排他源且左側情境下是負向因子；
E/G 與 F 之間有一條沒人守的縫（YoY 5-20%＋PE 15-30）；左側股轉換率掛零的根因是 F2 位階閘門
＋docs/11「左側永不入核心」政策，不在候選層。

**待排序 milestone**（依 docs/31 §9，尚未拍板執行順序，等使用者指示）：
1. docs/31 §7.4 驗證計畫（優先，任何候選 filter 生產化前的必要門檻——需獨立於 W21-33 的
   驗證窗、opus/verifier 複核前視偏誤）
2. G3（族群已轉強、個股還沒領先；零工程成本，欄位已存在）可較快單獨驗證上線
3. G1/G4/F2'（小工程量：擴 `field_map`/`universe` 少數欄）
4. §7.1 政策旁路（F2 閘門的機會層旁路；風險最高，需驗證通過才排，直接動 docs/11 SOP）
5. §8 資料擴充 queue（`t187ap45_L`/`t187ap11_L` 逐年累積可行性、`MI_INDEX` 市值加權指數

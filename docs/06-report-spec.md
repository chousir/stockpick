...
請依照 playbook/60-analyst-persona.md 的個股報告框架產出完整 Markdown。

特別注意：
1. 所有數字必須來自上面的資料區塊或 Goodinfo，不可從記憶估算
2. 多方論點 3-5 點，空方論點 3-5 點，空方不可少於多方
3. 進場條件給具體價位（從 MA20、MA60、區間高低點推算），並附邏輯失效訊號（多方論點已破的可觀察訊號，與停損價分開）
...
報告**絕對不能**出現：
1. 「目標價 XXX 元」、「會漲到 XXX」這種價格預測——**例外見 playbook/60「估值回歸
   參考價」條**：機械式回顧計算（現價×中位數/現行比值）不算預測，但輸出時仍須明確
   標「估值回歸參考價」＋固定免責句，不可用「目標價/預估價/會漲到」等字眼。
   另有 playbook/60「例外二・實驗區塊豁免」（docs/31 §20.13 Phase 2）：**僅 `pick.md`
   附錄 G 得用「目標價」字眼**——**此豁免不及於本規格治理的個股報告
   `reports/<週次>/stocks/*.md`**，亦不及於 pick.md 決策卡與 picks 區塊。個股報告三大
   禁區完全不變。

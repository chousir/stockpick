...
| **核心條件** | 週 MACD + 均線多頭 + 流動性 | EPS YoY + 營收 YoY + 外資連買 | ROE ≥ 15% + 連續配息 + PB ≤ 3.0 |
...
name: "成長主力"
description: "EPS 與營收雙引擎成長 + 外資布局中的主力中線股"
...
**取捨**：
- 月累計營收 YoY ≥ 15%：營收動能
- **最新季稅後純益年增率 ≥ 20%**：實質獲利成長（新增）。避免單純營收灌水但無利潤的票。
- **拿掉「投信連買」**：舊版「外資 + 投信雙連買」每週撈 < 10 檔太嚴，
  改用「EPS YoY 條件」補強質量；外資連買單一條件即可（投信通常跟外資）
...
id: c_quality_value
name: "品質價值"
description: "高 ROE + 連續配息 + 估值合理的長線品質股（防守 / 壓艙石）"
...
- 股價淨值比 ≤ 3.0：估值不在頂端，避免追高。**這是「價值」屬性的核心**
- 不設技術 rule：品質股不擇時，何時進場都能慢慢累積部位
- 預期撈出 30–60 檔（不像舊「穩健存股」的 200+ 檔，因為 ROE 條件相對嚴）

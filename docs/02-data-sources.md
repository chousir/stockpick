...
四個欄位合起來，剛好讓 **F（價值反彈）策略**四條件全部可由官方資料覆蓋。D 的近四季ROE、
股利連續配發次數，E/G 的單季淨利連增，這幾個 Goodinfo 原始欄位官方 API 確實**只回最新一季/
無歷史查詢**，物理上無法**同義**回補。

**2026-08-22 修正**：上一版在此斷語「非本輪範圍」——使用者審閱後駁回「等資料源解封」這個
框架，指示不追求在本地重建 Goodinfo 定義過的同一批欄位，改用全新本地 filter 設計。詳見
**docs/31**（D/E/F/G 全面脫離 Goodinfo 重新設計規劃書）。同一輪盤點也發現兩個先前誤判為
「不可得」的官方端點：**歷史股利分派**（`/v1/opendata/t187ap45_L`，含股利年度/現金股利/
本期淨利，但僅回最新出表日快照，多年紀錄需累積，是否可回查歷史月份未驗證）與**董監持股+
設質比**（`/v1/opendata/t187ap11_L`，含設質股數/設質比例，月頻，歷史深度未驗證）——
docs/proposals/02「董監持股完全不在資料層」的說法需一併修正，端點確實存在。

...
## 5. 族群輪動機械基準（非個股品質排名・非挑股池・前 10）
> ⚠️ 純系統公式排序（0.6×族群強度＋0.3×族群內排名＋0.1×多策略），不含個股動能/籌碼/估值；僅供族群輪動參考，挑股請看第 6 節深度解讀與 candidates_enriched.csv。
1. 3034 聯詠（族群 #1 半導體業・族群內 #1・A+B）
...

## 6. 領先次產業深度分析請求（給 Claude・雷達驅動）
